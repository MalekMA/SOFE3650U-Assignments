...
/**
 * The CUOITGuideIterator class allows us to traverse the CUOITGuide collection
 */
public class CUOITGuideIterator implements Iterator, java.util.Iterator<TVListings>{

	LinkedList<TVListings> myListings = new LinkedList<TVListings>();		// Creates a local Linked List
	int position = 0;								// Creates a local variable position to be used in iteration
	
	/**
	 * CUOITGuideIterator constructor
	 * @param  listings  the Linked List from CUOITGuide is needed as a parameter
	 */
	public CUOITGuideIterator(LinkedList<TVListings> listings){
		this.myListings = listings;						// Stores the Array List from CDCGuide into the local one
	}
	
	/**
	 * The next method gets the contents of the Linked List
	 * @return  listings  the listings that are stored in the Linked List
	 */
	 ...
	 * in the Linked List is empty
	 * @return false  if the current position in the Linked List is null
